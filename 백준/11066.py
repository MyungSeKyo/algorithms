...
def get_cost_by_memoization(chapters):
    memo = [[0] * len(chapters) for _ in chapters]
    stacked_sum = chapters.copy()
    for i in range(1, len(stacked_sum)):
        stacked_sum[i] += stacked_sum[i - 1]

    def _get_cost(left, right):
        if memo[left][right] > 0:
            return memo[left][right]
        elif left == right:
            return 0
        elif left + 1 == right:
            memo[left][right] = chapters[left] + chapters[right]
            return memo[left][right]
        else:
            min_cost = sys.maxsize
            for middle in range(left, right):
                min_cost = min(min_cost, _get_cost(left, middle) + _get_cost(middle + 1, right))

            if left == 0:
                min_cost += stacked_sum[right]
            else:
                min_cost += stacked_sum[right] - stacked_sum[left - 1]
            memo[left][right] = min_cost
            return min_cost

    return _get_cost(0, len(chapters) - 1)


def get_cost_by_for_loop(chapters):
    dp = [[0] * len(chapters) for _ in chapters]
    stacked_sum = chapters.copy()
    ...
    for j in range(len(chapters)):
        for i in reversed(range(j)):
            ...
            if i == 0:
                min_cost += stacked_sum[j]
            else:
                min_cost += stacked_sum[j] - stacked_sum[i - 1]
            dp[i][j] = min_cost
...
for _ in range(case_num):
    ...
    print(get_cost_by_memoization(chapters))
